...
class MultiLayerPerceptron:
    """
    Implementation of MLP using numpy from scratch
    """

    def __init__(self, layers_dim, optimizer='adam', initialization='he', learning_rate=0.0075):
    ...
    def initialize_parameters(self, method='he'):
        """
        Arguments:
        method -- string; the initialization method (zeros, random, he)
    ...
    def initialize_EMA(self, parameters):
        """
    ...
    def initialize_RMS(self, parameters):
        """
    ...
    def L_model_forward(self, X, parameters, keep_prob=1):
        """
        ...
        for l in range(1, L):
            ...
            A, cache = self.linear_activation_forward(A_prev, parameters['W' + str(l)], parameters['b' + str(l)],
                                                      activation='relu')
            A, masks['D' + str(l)] = self.dropout(A, keep_prob)
            caches.append(cache)
    ...
    def compute_cost(self, AL, Y, parameters=None, lambd=0):
        """
    ...
    def linear_activation_backward(self, dA, cache, activation, AL=None, Y=None):
        """
    ...
    def L_model_backward(self, AL, Y, caches, parameters, masks=None, keep_prob=1, lambd=0):
        """
    ...
    def update_learning_rate(self, learning_rate0, epoch_num, decay_rate=1):
        """
    ...
    def update_parameters(self, parameters, grads, learning_rate, optimizer='adam', EMA=None, RMS=None, beta1=0.9,
                          beta2=0.999,
                          counter=None):
        """
    ...
    def predict(self, X, y, parameters):
        ...
        # Forward propagation
        y_hat, caches, masks = self.L_model_forward(X, parameters)
    ...
    def fit(self, X, Y, optimizer=None, initialization=None, mini_batch_size=64, learning_rate=None,
            epochs=1000, print_cost=False, keep_prob=1, lambd=0, decay=None, decay_rate=1):
        """
        ...
        initialization = initialization if initialization else self.initialization
        learning_rate = learning_rate if learning_rate else self.learning_rate
        decay = decay if decay else self.update_learning_rate
        ...
        seed = 10  # seed
        minibatches = self.random_mini_batches(X, Y, mini_batch_size, seed)  # mini batches
        ...
        parameters = self.initialize_parameters(method=initialization)
        L = len(parameters) // 2  # Number of layers

        # Check the optimizer and initializing the requirements
        # Gradient descent with momentum
        if optimizer == 'momentum' or optimizer == 'adam':
            EMA = self.initialize_EMA(parameters)  # Exponentially Moving Averages

        # Root Mean Square propagation
        if optimizer == 'rmsprop' or optimizer == 'adam':
            RMS = self.initialize_RMS(parameters)  # Root mean square
        ...
        for i in range(0, epochs):
            ...
            for minibatch in minibatches:
                ...
                AL, caches, masks = self.L_model_forward(minibatch_X, parameters, keep_prob=keep_prob)

                # Compute cost.
                cost = self.compute_cost(AL, minibatch_Y, parameters=parameters, lambd=lambd)

                # Backward propagation.
                grads = self.L_model_backward(AL, minibatch_Y, caches, parameters, masks=masks, keep_prob=keep_prob,
                                              lambd=lambd)
                ...
                if optimizer == 'gd':
                    ...
                    parameters, EMA = self.update_parameters(parameters, grads, learning_rate, optimizer='momentum',
                                                             EMA=EMA)
                elif optimizer == 'rmsprop':
                    parameters, RMS = self.update_parameters(parameters, grads, learning_rate, optimizer='rmsprop',
                                                             RMS=RMS)
                elif optimizer == 'adam':
                    t += 1
                    parameters, EMA, RMS = self.update_parameters(parameters, grads, learning_rate, optimizer='adam',
                                                                  RMS=RMS,
                                                                  EMA=EMA, counter=t)
    ...
    def save_model(self, parameters, path):
        """
